...
public:
    Projectile(Vector2 pos, Vector2 vel, int dmg, WeaponType wt) {
        ...
        if (type == WeaponType::ROCKET && !rocketTextureLoaded) {
            if (FileExists("rocket.png")) {
                rocketTexture = LoadTexture("rocket.png");
                if (rocketTexture.id > 0) {
                    GenTextureMipmaps(&rocketTexture);
                    SetTextureFilter(rocketTexture, TEXTURE_FILTER_TRILINEAR);
                    rocketTextureLoaded = true;
                    TraceLog(LOG_INFO, "Rocket texture loaded successfully");
                }
            } else {
                TraceLog(LOG_WARNING, "Grzegorz_Braun.png file not found");
            }
        }
    }

    bool Update(float dt, std::vector<std::unique_ptr<Asteroid>>& asteroids) {
        if (type == WeaponType::ROCKET && target == nullptr) {
            float minDist = FLT_MAX;
            for (auto& ast : asteroids) {
                float dist = Vector2Distance(transform.position, ast->GetPosition());
                if (dist < minDist) {
                    minDist = dist;
                    target = ast.get();
                }
        ...
        if (type == WeaponType::ROCKET && target != nullptr) {
            Vector2 dir = Vector2Normalize(Vector2Subtract(target->GetPosition(), transform.position));
            physics.velocity = Vector2Scale(dir, 400.f);
        }
    ...
    void Draw() const {
        if (type == WeaponType::BULLET) {
            ...
            if (rocketTextureLoaded && rocketTexture.id > 0) {
                float rotation = atan2f(physics.velocity.y, physics.velocity.x) * RAD2DEG + 90;
                DrawTexturePro(
                    rocketTexture,
                    { 0, 0, (float)rocketTexture.width, (float)rocketTexture.height },
                    { transform.position.x, transform.position.y, 
                      (float)rocketTexture.width * 0.05f, (float)rocketTexture.height * 0.05f },
                    { (float)rocketTexture.width * 0.05f, (float)rocketTexture.height * 0.05f },
                    rotation,
                    WHITE
                );
                ...
                Vector2 tip = Vector2Add(transform.position, Vector2Scale(dir, 12.f));
                Vector2 left = Vector2Add(transform.position, Vector2Scale(Vector2Rotate(dir, PI * 0.8f), 8.f));
                Vector2 right = Vector2Add(transform.position, Vector2Scale(Vector2Rotate(dir, -PI * 0.8f), 8.f));
    ...
    float GetRadius() const {
        switch (type) {
            ...
            case WeaponType::ROCKET: return 8.f;
            default: return 0.f;
        }
    }
...
Texture2D Projectile::rocketTexture;
bool Projectile::rocketTextureLoaded = false;
    ...
    void Run() {
        ...
        while (!WindowShouldClose()) {
            ...
            auto projectile_to_remove = std::remove_if(projectiles.begin(), projectiles.end(),
                [dt, this](auto& projectile) {
                    return projectile.Update(dt, asteroids);
                });
